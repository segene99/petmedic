...
//	@ResponseBody
//	public List<HospitalVO> getHosList(HospitalVO vo, Model model) {
...
//			모델 어트리뷰트 써서 끌고올것
		...
		@RequestMapping("/pickHos")
	      public String pickHos(UsersVO uvo, Model model, HttpSession session) {
	         ...
	         if(((String)userService.getUserInfo(uvo).getUsers_pick1())==null ||((String)userService.getUserInfo(uvo).getUsers_pick2())==null||((String)userService.getUserInfo(uvo).getUsers_pick3())==null||((String)userService.getUserInfo(uvo).getUsers_pick4())==null||((String)userService.getUserInfo(uvo).getUsers_pick5())==null) {
	            System.out.println("왔냐");
	            if(!((uvo.getHos_id()).equals((String)userService.getUserInfo(uvo).getUsers_pick1()) || (uvo.getHos_id()).equals((String)userService.getUserInfo(uvo).getUsers_pick2()) || (uvo.getHos_id()).equals((String)userService.getUserInfo(uvo).getUsers_pick3()) || (uvo.getHos_id()).equals((String)userService.getUserInfo(uvo).getUsers_pick4()) || (uvo.getHos_id()).equals((String)userService.getUserInfo(uvo).getUsers_pick5()))){
	                         ...
	                         if(((String)userService.getUserInfo(uvo).getUsers_pick2()!=null)){
	                            if(((String)userService.getUserInfo(uvo).getUsers_pick3()!=null)){
	                               if(((String)userService.getUserInfo(uvo).getUsers_pick4()!=null)){
	                                  ...
	                                  } else {userService.pickHos5(uvo);}
	                               } else { userService.pickHos4(uvo);}
	                            } else {userService.pickHos3(uvo);}
	                         }else {userService.pickHos2(uvo);}
	                     } else {userService.pickHos1(uvo);}
	                  }else {
	...
	// [관리자] 병원회원탈퇴 수빈유지보수
	   ...
	   public String deletewaitingHospital(HospitalVO hosvo, Model model) {
	      hospitalService.deletewaitingHospital(hosvo);
				...
				System.out.println("hosvo의 병원아이디값" + hosvo.getHos_id());
	      hospitalService.delHosZzim(hosvo);
			...
			// 아이디 중복체크
				...
				// 베스트 병원 목록으로 가져오기
				   ...
				   public String step1() throws Exception{
					  System.out.println("beforeHosJoin 컨트롤러 ");
