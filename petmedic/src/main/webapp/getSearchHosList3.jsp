...
<meta charset="utf-8">
<title>병원검색</title>
<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=25731966bd6475ae2f7635d90222da92&libraries=services,clusterer,drawing"></script>
...
.searchResultInfoHere:hover, #searchResultInfoHere:active,
	#searchResultInfoHere:focus {
	cursor: pointer;
	transition: background-color 0.3s ease, transform 0.2s ease;
}
...
// 		    imageSize = new kakao.maps.Size(64, 69), // 마커이미지의 크기입니다
        ...
        function getHospitalList(keyword) {
            $.ajax({
                ...
                type: 'post',
                dataType: 'json',
                data: { keyword: keyword }, // Pass the keyword as data to the server
                async: false,
        ...
        function geocodeAddress(number,hospital, map) {
            ...
            geocoder.addressSearch(address, function (result, status) {
                if (status === kakao.maps.services.Status.OK) {
					...
					// 툴팁을 노출하는 마커를 생성합니다.
					var marker1 = new TooltipMarker(coords, name);
					marker1.setMap(map);
					
					// MarkerTracker를 생성합니다.
					var markerTracker1 = new MarkerTracker(map, marker1, name);
                 ...
                 // 커스텀 오버레이를 지도에 표시합니다
					...
					if (address) {
					    itemStr +=  '<div onclick="moveToMarker(this)">'
					            + '<div class="addr_2_map">' + address + '</div>';
					} else {
					    itemStr += '<div class="jibun gray">' + address_jibun + '</div>';
					}
					...
					itemStr += '<div style="display: inline;" class="star-rating" id="st-' + number + '">'
					    ...
					    + hos_star
					    + '</span>'
					    + '<div>'+ '<b>전문진료</b>: ' + hos_cate +'</div>'
					    + '<div>'
					    + '  <span>'+ '<b>진료시간</b>: ' + hos_start + '-' + hos_end + '</span>'
					    + ' |'
					    + '  <span>'+ '<b>휴일</b>: ' + hos_hol + '</span>'
					    + '</div>'
					    + '</div>'
					    + '</div>'
					    + '<hr>';
		...
		function moveToMarker(divElement) {
				 ...
				 var clickedDivValue = divElement.querySelector('.addr_2_map').innerHTML;
			      ...
			      var geocoder = new kakao.maps.services.Geocoder();
			      geocoder.addressSearch(clickedDivValue, function(result, status) {
			        if (status === kakao.maps.services.Status.OK) {
			          var coords = new kakao.maps.LatLng(result[0].y, result[0].x);
			          console.log(coords);
			          console.log(map);
	...
	<script>
		...
		//이전별점
         function generateStarRating(idx, starValue) {
           ...
           for (var i = 1; i <= 5; i++) {
           var starHtml = '';
             starHtml += '<input type="radio" id="' + i + '-stars" name="hos_star" value="' + i + '"';
